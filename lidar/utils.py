...
def find_matched_gt_box(pred_box, gt_bboxes):
    iou_scores = [compute_iou(pred_box, gt_box) for gt_box in gt_bboxes]
    max_iou_score = max(iou_scores)
    matched_gt_box = iou_scores.index(max_iou_score)
    return max_iou_score, matched_gt_box


def evaluate_metrics(gt_bboxes, predicted_bboxes, iou_threshold=0.5):
    tp, fp, fn = 0, 0, 0
    matched_gt_boxes = set()
    for pred_box in predicted_bboxes:
        max_iou_score, matched_gt_box = find_matched_gt_box(pred_box, gt_bboxes)
        if max_iou_score >= iou_threshold:
            ...
            fp += 1
    fn = len(gt_bboxes) - len(matched_gt_boxes)
